...
Example showing various read and write methods using the MicrochipSRAM
library\n\n

The library uses the standard SPI library and assumes that the default hardware
SPI is being utilized.\n\n

The library contains the constructor, which requires the CS/SS pin number as
input and returns the size, in bytes, of the specific Microchip SPI SRAM chip
detected. If a zero is returned it means that no chip was detected which could
be caused by either an incorrect CS/SS pin number, or incorrect wiring or no
chip.\n\n

There are only 3 public methods in the library:\n\n ** [optional uint32 return
address] get ( address, {variable} )\n [optional uint32 return address] put (
address, {variable} )\n fillMemory ( address, {variable} )\n\n If the address is
higher than the actual amount of memory bytes availabe it automatically
overflows and starts back at the beginning of memory, i.e. a 256kbit chip
(23x256) stores 32768 bytes. If one puts 2 bytes starting at 32767 then byte 0
of the array is overwritten. Get and Put will word wrap from the end to the
beginning.\n\n

fillMemory will place identical copies of the {variable} starting at the memory
position specified to the end of memory. Any partial entry at the end of memory
will not be filled.
